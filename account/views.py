...
def pending_orders(request):
    try:
        #Seleccionar ordenes sin pago 'No pagada' = 'New' de todos los usuarios
        # Sí funciona: orders = Order.objects.order_by('-created_at').filter(user_id=request.user.id, is_ordered=True, paid_at=None)
        orders = Order.objects.order_by('-created_at').filter(is_ordered=True, status="New")
        # Sólo los orderproducts de las ordenes consultadas
